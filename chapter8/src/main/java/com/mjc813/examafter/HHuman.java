        ...
        for (int i = 0; i < hospital.getParents().toArray().length; i++) {
            if (name.equals(hospital.getParents().get(i).getName()) && id.equals(hospital.getParents().get(i).getId())) {
                if (hospital.getParents().get(i).getMaster() != this) {
                    System.out.println(hospital.getParents().get(i).getType() + " " + hospital.getParents().get(i).getName() + "의 주인은 " + this.getName() + "이 아니므로 만날 수 없습니다.");
                    return;
                }
                if (hospital.getParents().get(i).getVisit() >= 1) {
                    System.out.println(hospital.getParents().get(i).getType() + " " + hospital.getParents().get(i).getName() + "은 이미 오늘 만났습니다.");
                    return;
                }
                ...
                System.out.println(this.getName() + "의 애완동물 환자 "  + hospital.getParents().get(i).getType() + " " + hospital.getParents().get(i).getName() + "와 면회했습니다.");
